...
class ir_attachment(osv.osv):
    ...
    def _full_path(self, cr, uid, path):
        ...
        path = path.strip('/\\')
        print "2=======================", path
        print "3=======================", os.path.join(self._filestore(cr, uid), path)
    ...
    def _filestore(self, cr, uid, context=None):
        print "filestore====================", tools.config.filestore(cr.dbname)
    ...
    def _file_read(self, cr, uid, fname, flag, bin_size=False):
        if flag == 0:
            full_path = '/var/www/html/prime_erp_api/storage/app/prime_builders/'+fname
        else:
            full_path = self._full_path(cr, uid, fname)
    ...
    def _data_get(self, cr, uid, ids, name, arg, context=None):
        ...
        for attach in self.browse(cr, uid, ids, context=context):
            if attach.store_fname:
                print "store_fname======================", attach.store_fname
                result[attach.id] = self._file_read(cr, uid, attach.store_fname, 0, bin_size)
                print "result[attach.id]==================", result[attach.id]
            elif attach.store_fname_new:
                result[attach.id] = self._file_read(cr, uid, attach.store_fname_new, 1, bin_size)
            else:
    ...
    def _data_set(self, cr, uid, id, name, value, arg, context=None):
        ...
        if location != 'db':
            ...
            super(ir_attachment, self).write(cr, SUPERUSER_ID, [id], {'store_fname_new': fname, 'file_size': file_size, 'db_datas': False}, context=context)
        else:
    ...
    _columns = {
        ...
        'store_fname_new': fields.char('Stored Filename'),
